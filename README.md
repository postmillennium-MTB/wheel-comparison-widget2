...
**Simplified mode.** The toggle at the top right strips the page back to one wheel, one hub picker, and one number, for readers who want the answer rather than the workings. It hides the shared build settings, both sides' config cards, the headline verdict card, all six detail cards, the bottom-line paragraph, and the method note. What it keeps is the condition strip (`29" · 32H · 120 kgf DS · same rim both sides · 5.06° drive-side bracing`), so the number stays self-explanatory even with the controls it was computed from out of view. Nothing leaves the DOM — every hidden control stays wired and keeps feeding the engine, so switching back shows a fully current full view, and the hub you pick in either view is the same state. The mode rides in the shareable link as `sm=148` or `sm=157`, naming the standard the wheel was showing, so a simplified comparison can be sent as one.

**Either side can hold either standard.** The big `148x12` / `157x12` label at the top of each side card is a button, not a caption — click it and that side switches standards, landing on the same position in the other catalogue so you get a comparable hub rather than the top of the list. Both hub dropdowns also list all 30 hubs grouped by standard, so picking across the groups does the same thing; that is how a wheelbuilder thinks about it anyway, since you choose a hub and its standard follows. This is what makes a 148-vs-148 (or 157-vs-157) comparison possible.

In simplified mode the two buttons choose which side you are *looking at*, so a separate "⇄ Switch this side to …" control under them changes what that side *is*. Without it the only route to a same-standard comparison was noticing the optgroups in the hub list, which nobody does — the obvious move is to click the standard you can see, and that button was selecting a side rather than switching one.

A side's standard is *derived* from the hub it is holding rather than stored next to it, so the two can never disagree, and shared links keep working unchanged — they already carried hub ids. When both sides land on the same standard, every piece of copy that would have named it (the hero badge, the six per-metric verdicts, the bottom line, the hero subtitle, simplified mode's comparison line) names the hub instead, because at that point the standard no longer distinguishes anything.

One naming wart worth knowing: the `148`/`157` in element ids and in `ST.hub148` / `ST.hub157` is historical. Those now identify the two comparison *slots* — amber and blue — not the standards they hold. Renaming 28 ids on a live tool was judged riskier than documenting the convention.

**The wheel spins.** Continuously, one revolution every 14 seconds — slow enough to read as a machine idling rather than something demanding attention. A single `requestAnimationFrame` loop drives the spin and the geometry tween together; two loops would double the paint rate and let them tear. The loop is parked whenever it would be wasted work: the mode is closed, the tab is backgrounded, or the stage has scrolled off screen (`IntersectionObserver`). Under `prefers-reduced-motion: reduce` it never starts — a permanent animation is precisely what that setting exists to stop. The bracing-angle annotation stays at twelve o'clock while the spokes turn under it: it marks where the angle is measured, so pinning it keeps the number readable instead of carrying it around the rim.

**One wheel, not two.** This is deliberate, and it is the opposite of what a comparison tool would suggest. Two wheels side by side at honest scale look identical: the difference between a 148 and a 157 build is roughly one degree of bracing angle (5.06° vs 6.06° for the two Chris King hubs at 29"/32H), which is imperceptible on a 600 mm wheel. A side-by-side picture would therefore argue *against* the 20–30% strength gap printed underneath it, and the only way to make the two look different is to inflate the drawing past the angle being quoted. Switching one wheel between the standards puts the same change on the channel the eye is actually sensitive to — motion — and the geometry, the bracing angle, the headline number, and the series color all tween together over 380 ms off one clock. `prefers-reduced-motion: reduce` skips the tween and lands on the final state.

**The wheel drawing** is ported from the PMR homepage's PLATE I: a wireframe wheel on a canvas with no library, N spokes, two flanges and a rim, rotated by a plain 3×3 and projected. Two things differ from the homepage version. First, the model: the homepage has no hub catalogue, so it derives both flange offsets from one spacing number using a fixed NDS ratio and a single PCD, whereas here each hub's real `nds`, `ds`, `pds` and `pnds` drive the drawing, the two flanges get their own radii, and the rim radius is the real ERD/2 — so changing wheel size changes the picture the way it changes the physics. Second, the drag: on the homepage, dragging spreads the hub, because spacing is the free variable there; here spacing is fixed by the hub you picked, so dragging it would contradict the number on screen. Drag orbits the camera instead, which is the gesture worth having anyway — a bracing angle is invisible head-on, and turning the wheel is what brings it into view. The homepage's `VIS_SPREAD = 1.8` drawing exaggeration carries over, and as there it is drawing-only: every quoted angle uses the true offsets.

One thing the drawing invites readers to over-conclude, and which the catalogue does not support: symmetry alone does not make a wheel strong. The most symmetric 148 in the list (KOM Xeno Rear Boost, 77.2% balance) is also the softest laterally (K_lat 43.0), because it buys that symmetry with a narrow 54.0 mm flange spread and a 46 mm PCD — against DMR Zone's 55.6% balance and K_lat 68.9. Flange spread and PCD set how much lateral stiffness is available; symmetry decides how much of it survives to the non-drive side. The tagline "symmetry = strength" is a slogan, not the model.
...
**Simplified is the default on a narrow screen.** On a bare visit — no hash at all — the tool opens simplified below 700px and full at or above it. The trigger is width rather than device class on purpose: "is this a phone" is not answerable in a browser (an iPhone in landscape and an iPad in portrait are the same number of pixels), whereas "is there room for the full layout" is exactly the question that matters, and it also catches a laptop with a half-width browser window. 700px puts every phone in portrait into simplified and leaves tablets and up on the full view; the PMR page frames this tool at `width:100%`, so the measurement is the real viewport either way.

Any URL carrying state overrides the rule outright — a shared full-mode link opens full on a phone, because reproducing what the sender saw is the whole point of "Copy link". The auto-applied mode is also not written into the address bar until the reader touches something, so a phone visitor who copies the link does not silently pass on a decision they never made. When the mode was applied by the rule rather than chosen, a "Show the full breakdown" link sits at the foot of the stage; without it a first-time mobile visitor would see a wheel, a number and a sentence and reasonably conclude that is the whole tool. Choosing the mode by hand retires the hint.

**`#simple` opens simplified mode.** There are two kinds of link into this tool. "Copy link" produces the long one — a `key=value` hash that reproduces one exact comparison, hub by hub. `#simple` is the short one: it just names a mode and leaves everything else at its defaults, which is what you want in a link, a QR code, or an iframe `src` when all you need is for the tool to open simplified. It is recognised at load and on `hashchange`, so typing it onto the end of an already-open page works too (`history.replaceState`, which the long form uses, does not fire `hashchange`, so the two never fight). Case-insensitive. A visitor who arrives on `#simple` keeps that URL until they change something — expanding it into a 130-character state blob the moment the page painted would quietly replace the link they were handed. The first interaction unlocks it and normal link-sharing resumes. `#sm=148` / `#sm=157` still open simplified mode on a named standard.
...
| e*thirteen, *Sidekick Hub Dimensions for Wheelbuilding* | Both e*thirteen Sidekick entries | [support.ethirteen.com](https://support.ethirteen.com/hc/en-us/articles/29875435477787-Sidekick-Hub-Dimensions-for-Wheelbuilding) |
...
What the conversion *does* change is the cassette's position relative to the frame. Symmetric caps leave the cassette where it sat on the 150, so a converted hub does not inherit a native Super Boost chainline, and frame and drivetrain clearance should be checked against the specific bike rather than assumed from the axle width. That is a fitment question, not a structural one, and it sits outside what this widget computes — but it is the reason a converted 150 and a native 157 SB were two different rows here rather than interchangeable ones.

**Removal (2026-09).** The Hope Pro5 150→157 conversion entry described above has been removed from the catalogue. At 95.8% NDS/DS ratio it sat well clear of every other 157 in this dataset (next-highest is 81.6%) — not because it is an exceptionally well-balanced conventional 157 build, but because a 150 shell converted with endcaps is barely dished to begin with, which is a different thing to measure than the rest of this table compares. Left in, it skewed the 157 side's range and made the tension-balance comparison read as a recommendation for a build that isn't really a "157" in the sense the rest of the catalogue means. The sourcing and correction history above is kept as a record of the work already done on this hub, in case it's ever reintroduced with that context attached.
...
**Addition (2026-09), e\*thirteen.** Two entries came from e\*thirteen's own support-article dimension table, *"Sidekick Hub Dimensions for Wheelbuilding"*: **Sidekick Rear MTB Hub V2 148x12 Boost** (part numbers HB5UKA-204 through -209) and **Sidekick Pro Rear MTB Hub V2 157x12 Superboost** (HB5UKA-222, -223, -226, -227). The same article also lists a non-Pro "Sidekick Rear MTB Hub V2 157x12 Superboost" (HB5UKA-210 through -215) with identical published geometry to the Pro version — it was left out simply because it wasn't the one asked for, not for any data reason, and it can be added later without new sourcing work if wanted.

The article's table gives "Flange Diameter Left/Right" and "Center to Flange Left/Right" per model, with no column stating which side is drive vs non-drive. Unlike the DT Swiss and KOM entries above, where the source document itself supplied an independent figure (a mid-flange offset, a front-hub row) that confirmed the mapping, no such internal check exists here. Left is entered as non-drive and right as drive on the strength of two things instead: the near-universal MTB convention that the drive side is the right side of the bike, and internal consistency with every other hub already in this dataset — Left > Right on both new rows (38.0 > 21.0 at 148, 42.5 > 25.5 at 157), matching the NDS > DS pattern every other hub here shows, where a reversed mapping would have shown the opposite. That is corroboration, not confirmation from the source itself, so caliper measurements against a physical Sidekick hub would be a genuine addition of information here, more so than for the other recent entries.

Both Sidekick entries share one flange diameter, 60 mm on both sides — geometry contributes to the tension-balance ratio only through the two *offsets* on these hubs, not through a pitch-circle difference the way it does on, say, DT Swiss or Project 321.
...
**The 150 row is not irrelevant to the 157 side, though — which is the subtler half of this.** Hope's 150 shell converts to 157x12 with 3.5 mm endcaps per side, so it is a real 157x12 option, and it was added to the catalogue in its own right as "Hope Pro5 150 6 bolt (157 conv)" (since removed as an outlier — see "Removal (2026-09)" above). The v1 error was therefore not "used a 150 hub, which has no business here"; it was collapsing two genuinely different 157-capable Hope hubs into one entry and attaching the wrong geometry to the wrong name. Both belonged, as separate rows with separate numbers. A catalogue name that merges two axle standards is a latent sourcing bug precisely because the merge hides a real distinction rather than inventing a fake one.
...
| e*thirteen Sidekick 148 Boost | 148 | 55.6% | 76.7 | 4741 | 155.3 | not yet run |
...
| e*thirteen Sidekick Pro 157 SB | 157 | 60.4% | 97.6 | 4725 | 181.7 | not yet run |
...
**DT Swiss 350 — a bigger flange on the side that needs it.** Almost every hub here uses a drive-side flange as large as or larger than the non-drive side: 15 of the 28 entries preceding this pair had the larger flange on the drive side, 11 ran them equal, and only OneUp went the other way. DT joins OneUp as the exception, and does it at both widths — 58 mm non-drive against 52 mm drive at 148, 60 against 50.5 at 157. A larger non-drive flange lengthens the lever those spokes pull on, which is the side a dished rear wheel has least to work with. The 157 lands at 96.0 N/mm on a 67.9% balance, a stronger combination than most of the group, without resorting to the deep dish of the DMR.
...
What that buys is less than it might appear. Both land mid-pack for lateral stiffness, not at the top: the 148 is 7th of 15 (76.2 N/mm) and the 157 is 11th of 15 (91.2 N/mm). So the deep dish is not purchasing class-leading stiffness — the I9 Hydra Centerlock 148 is stiffer (82.6 N/mm) while carrying a far healthier 62.2% balance.

The broader pattern in the 148 group is close to monotonic in the other direction. Ranked by stiffness, the top of the table runs 62–68% balance and the bottom runs 69–77%: project 321 (57.9 N/mm, 69.1%) and the KOM Xeno (53.6 N/mm, 77.2%) sit last on stiffness and first on balance. Across this catalogue the two metrics trade against each other far more often than they move together — which is the case for showing four numbers rather than one score.

**Axle standard is not the only driver.** The 148 range (53.6–82.6 N/mm) and 157 range (63.0–102.5 N/mm) overlap substantially — the bottom quarter of the 157 range sits inside the 148 range entirely. A well-specified 148 build with a high-quality hub and large-flanged hubs can outperform a poorly specified 157 build. What the 157 standard enables is a structural ceiling that is simply not available at 148 mm — the best 157 hubs reach about 25% higher lateral stiffness than the best 148 hubs in this dataset.
